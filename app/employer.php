...
if ($db) {
    ...
    try {
        ...
        $totalCandidates = 0;
        foreach ($jobs as $j)
            $totalCandidates += (int) ($j['nb_candidates'] ?? 0);
    } catch (PDOException $e) {
        $jobs = [];
        $totalCandidates = 0;
    }
        ...
        <main class="app-main">
            ...
            <div class="app-main-content layout-app layout-app-wide">
                ...
                <div class="action-bar">
                    ...
                    <?php if ($hasPositions): ?>
                        <a href="employer-job-create.php" class="btn btn-primary">+ Créer un affichage</a>
                    <?php else: ?>
                        <a href="employer-positions.php" class="btn btn-primary">+ Créer mon premier poste</a>
                    <?php endif; ?>
                </div>
                ...
                <?php else: ?>
                    <div class="job-list">
                        <?php foreach ($jobs as $job):
                            ...
                            ?>
                            <a href="employer-job-view.php?id=<?= $job['id'] ?>" class="job-card-employer"
                                data-status="<?= htmlspecialchars($jStatus) ?>">
                                <div class="job-info">
                                    <h3><?= htmlspecialchars($job['title']) ?></h3>
                                    <div class="job-meta"><?= $job['nb_candidates'] ?? 0 ?> candidat(s) · <span
                                            class="job-status job-status-<?= $jStatus ?>"><?= $statusLabels[$jStatus] ?? $jStatus ?></span>
                                    </div>
                                </div>
