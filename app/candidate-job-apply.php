...
<body>
    <div class="app-shell">
        <?php $sidebarActive = 'jobs';
        include __DIR__ . '/includes/candidate-sidebar.php'; ?>
        <main class="app-main">
            ...
            <div class="app-main-content layout-app">
                ...
                <?php if ($alreadyApplied): ?>
                    <div class="success"
                        style="padding:1rem;background:#efe;border:1px solid #cfc;border-radius:8px;margin-bottom:1rem;color:#060;">
                        ✓ Vous avez déjà postulé à cette offre. <a href="candidate-applications.php"
                            style="color:var(--primary);">Voir mes candidatures</a>
                    </div>
                <?php elseif ($applicationWithdrawn): ?>
                    <div
                        style="padding:1rem;background:var(--bg-alt,#f8fafc);border:1px solid var(--border,#e5e7eb);border-radius:8px;margin-bottom:1rem;color:var(--text-secondary);">
                        Vous vous êtes retiré de cette candidature. Vous pouvez postuler à nouveau ci-dessous si vous le
                        souhaitez.
                    </div>
                <?php endif; ?>
                ...
                <div class="job-card">
                    ...
                    if ($hasLocation): ?>
                        <div class="job-apply-location"
                            style="margin-bottom:1rem;padding:0.75rem 1rem;background:var(--bg-alt, #f3f4f6);border-radius:8px;font-size:0.9rem;">
                            <strong>📍 Lieu du poste</strong>
                            <?php if (!empty($job['location_name'])): ?>
                                <div style="margin-top:0.25rem;color:var(--text-secondary, #6B7280);">
                                    <?= htmlspecialchars($job['location_name']) ?></div>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                    <?php if (!empty($job['description'])): ?>
                        <div class="desc" style="margin-bottom:1rem;"><?= nl2br(htmlspecialchars($job['description'])) ?>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <?php else: ?>
                    <?php if ($applicationWithdrawn): ?>
                        <p style="margin-bottom:1rem;color:var(--text-secondary);font-size:0.95rem;">Postuler à nouveau à cette
                            offre :</p>
                    <?php endif; ?>
                        ...
                        <div id="videoRecordBlock">
                            <div class="video-container">
                                ...
                                <div id="transferOverlay" class="transfer-overlay hidden">
                                    ...
                                    <div
                                        style="width:80%;background:#374151;height:10px;border-radius:5px;overflow:hidden;">
                                        <div id="uploadProgressBar"
                                            style="width:0%;height:100%;background:var(--success, #16a34a);transition:width 0.2s;">
                                        </div>
                                    </div>
        ...
        <script>
            ...
            async function initCamera() {
                try {
                    ...
                    btnStart.textContent = '📷 Autoriser la caméra';
                }
            }
